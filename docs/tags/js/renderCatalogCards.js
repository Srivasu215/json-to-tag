        ...
        card.innerHTML = `
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 pb-2 border-bottom">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="./tag.html?tag=${tag}" class="btn btn-outline-primary btn-sm py-0 px-2 small">
                        <i class="bi bi-book me-1"></i>Story &amp; Rules
                    </a>
                </div>
            ...
            <div class="row g-3 mt-1 small">
                <div class="col-md-6">
                    <span class="fw-bold text-muted text-uppercase d-block mb-1">
                        Allowed Attributes (${allowedAttrs.length}):
                    </span>
                    ${allowedAttrs.length > 0 
                        ? allowedAttrs.map(attr => `<span class="attr-pill">${attr}</span>`).join("")
                        : '<span class="text-muted fst-italic">Standard attributes (id, class, style)</span>'}
                </div>
                <div class="col-md-6">
                    <span class="fw-bold text-muted text-uppercase d-block mb-1">
                    ...
                    ${childTags.length > 0 
                        ? childTags.map(child => `<a href="./tag.html?tag=${child}" class="badge bg-secondary-subtle text-secondary font-monospace text-decoration-none me-1">&lt;${child}&gt;</a>`).join("")
